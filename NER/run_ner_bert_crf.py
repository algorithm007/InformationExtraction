...
'''
@Project  : DjangoBlog
@File     : run_ner_bert_crf.py
@IDE      : PyCharm
@Author   : 算法小学僧
@Date     : 2022/12/5 02:09 
'''
import os
import glob
import time
...
from tools.helpers import init_logger, seed_everything, collate_fn
...
def evaluate(args, model, tokenizer, prefix="", logger=None):
...
def predict(args, model, tokenizer, prefix="", logger=None):
...
def main():
    ...
    args.label2id = {label: i for i, label in args.id2label.items()}
    ...
    # training
    if args.do_train:
        train_dataset = load_and_cache_examples(args, args.task_name, tokenizer, data_type='train', logger=logger)
        global_step, tr_loss = train(args, train_dataset, model, tokenizer, logger=logger)
